...
// Sanitize and enforce ticket limits server-side: 3 free/Lite, 5 Pro.
...
`  const previousTickets = current.tickets || {};
  ...
  const rawTicketOptions = Array.isArray(body.tickets?.ticketOptions) && body.tickets.ticketOptions.length
    ...
    : [{
        id: 'soporte',
        label: body.tickets?.panelButtonLabel || previousTickets.panelButtonLabel || 'Soporte',
        emoji: body.tickets?.panelButtonEmoji || previousTickets.panelButtonEmoji || '🎫',
        description: 'Ayuda general',
      }];
  const seenTicketOptionIds = new Set();
  const cleanTicketOptions = rawTicketOptions.slice(0, ticketOptionLimit).map((item, index) => {
    const option = item && typeof item === 'object' ? item : {};
    let optionId = str(option.id || \`opcion-\${index + 1}\`, 32)
      .toLowerCase()
      .replace(/[^a-z0-9_-]/g, '');
    if (!optionId || seenTicketOptionIds.has(optionId)) optionId = \`opcion-\${index + 1}\`;
    seenTicketOptionIds.add(optionId);
    return {
      id: optionId,
      label: str(option.label || \`Ticket \${index + 1}\`, 80).trim() || \`Ticket \${index + 1}\`,
...
// Replace ticket panel payload with buttons/select menu support.
const payloadPattern = /function ticketPanelPayload\(settings = \{\}\) \{[\s\S]*?\n\}\n\nexport async function syncTicketPanel/;
if (payloadPattern.test(discord) && !discord.includes('function normalizedTicketOptions(settings = {})')) {
  discord = discord.replace(payloadPattern, String.raw`function normalizedTicketOptions(settings = {}) {
  const source = Array.isArray(settings.ticketOptions) && settings.ticketOptions.length
    ? settings.ticketOptions
    : [{
        id: 'soporte',
        label: settings.panelButtonLabel || 'Soporte',
        emoji: settings.panelButtonEmoji || '🎫',
        description: 'Ayuda general',
      }];

  return source.slice(0, 5).map((option, index) => ({
    id: String(option?.id || `opcion-${index + 1}`).replace(/[^a-z0-9_-]/gi, '').slice(0, 32) || `opcion-${index + 1}`,
    label: String(option?.label || `Ticket ${index + 1}`).slice(0, 80),
...
function ticketPanelPayload(settings = {}) {
  ...
  if (settings.panelMode === 'select') {
    ...
    for (const option of options) {
      ...
      if (option.emoji) {
        try { item.setEmoji(option.emoji); } catch {}
      }
...
// Route ticket select menus and reason modals.
...
if (!discord.includes('async function handleTicketSelect(interaction)')) {
  discord = discord.replace(
    'async function createTicketFromInteraction(interaction) {',
String.raw`async function handleTicketSelect(interaction) {
...
async function handleTicketModal(interaction) {
  ...
  await createTicketFromInteraction(interaction, { optionId, reason });
}
...
async function beginTicketOpen(interaction, optionId = '') {
  ...
  if (!option) {
    await interaction.reply({ content: 'Esa opción de ticket ya no existe. Actualiza el panel e inténtalo otra vez.', ephemeral: true });
    return;
  }

  if (!settings.tickets.requireReason) {
    await createTicketFromInteraction(interaction, { optionId: option.id, reason: '' });
    return;
  }
...
async function createTicketFromInteraction(interaction, { optionId = '', reason = '' } = {}) {`
  );
}

// Replace the legacy ticket creation body with category/reason aware flow.
const createPattern = /async function createTicketFromInteraction\(interaction, \{ optionId = '', reason = '' \} = \{\}\) \{[\s\S]*?\n\}\n\nasync function closeTicketFromInteraction/;
if (createPattern.test(discord) && !discord.includes("const optionLabel = option?.label || 'Soporte';")) {
  discord = discord.replace(createPattern, String.raw`async function createTicketFromInteraction(interaction, { optionId = '', reason = '' } = {}) {
  ...
  const optionLabel = option?.label || 'Soporte';
  const safeType = optionLabel.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '').slice(0, 18) || 'ticket';
  ...
  const channel = await interaction.guild.channels.create({
    ...
    reason: `Ticket ${optionLabel} de ${interaction.user.tag}`,
  });
  ...
  const openEmbed = new EmbedBuilder()
    .setTitle(`Ticket · ${optionLabel}`.slice(0, 256))
    ...
    .addFields({
      name: 'Motivo',
      value: cleanReason || (settings.tickets.requireReason ? 'No especificado' : 'No requerido'),
    });
...
// Configurable anti-spam thresholds and stronger link detection.
discord = discord.replace(
...
if (ticketsFormPattern.test(main) && !main.includes('const maxOptions = premiumPlan === \'pro\' ? 5 : 3;')) {
  main = main.replace(ticketsFormPattern, String.raw`function TicketsForm({ config, set, resources, premiumPlan }) {
  ...
  const updateOption = (index, key, value) => {
    const next = options.map((item, itemIndex) => itemIndex === index ? { ...item, [key]: value } : item);
    set('ticketOptions', next);
  };
  const removeOption = (index) => {
    if (options.length <= 1) return;
    set('ticketOptions', options.filter((_, itemIndex) => itemIndex !== index));
  };
  const addOption = () => {
    if (options.length >= maxOptions) return;
    const suffix = `${Date.now().toString(36)}-${options.length + 1}`;
    set('ticketOptions', [...options, { id: `ticket-${suffix}`, label: `Ticket ${options.length + 1}`, emoji: '🎫', description: '' }]);
  };
  ...
  return <div className="form-card ticket-builder">
    ...
    <FormSection title="Comportamiento" description="Elige cómo se abren los tickets.">
      ...
      <ToggleRow title="Pedir motivo antes de abrir" description="Si está activo, Discord mostrará una ventana para escribir el motivo antes de crear el canal." checked={!!config.requireReason} onChange={(value) => set('requireReason', value)} />
    </FormSection>
    ...
    <FormSection title="Opciones para abrir ticket" description={`Puedes usar hasta ${maxOptions} opciones con tu plan ${planName}.`}>
      <div className="ticket-option-stack">
        {options.map((option, index) => <div className="ticket-option-editor" key={option.id || index}>
          <div className="ticket-option-editor-head"><strong>Opción {index + 1}</strong><button type="button" onClick={() => removeOption(index)} disabled={options.length <= 1}>Eliminar</button></div>
          <div className="ticket-option-fields">
            <Field label="Texto"><input maxLength="80" value={option.label || ''} onChange={(event) => updateOption(index, 'label', event.target.value)} placeholder="Soporte" /></Field>
            <Field label="Emoji"><input maxLength="64" value={option.emoji || ''} onChange={(event) => updateOption(index, 'emoji', event.target.value)} placeholder="🎫" /></Field>
          </div>
          <Field label="Descripción" hint={config.panelMode === 'select' ? 'Se muestra dentro del menú desplegable.' : 'Se muestra al abrir el ticket.'}><input maxLength="100" value={option.description || ''} onChange={(event) => updateOption(index, 'description', event.target.value)} placeholder="Ayuda general" /></Field>
        </div>)}
      </div>
    ...
    <PreviewCard title="Vista previa">
      <div className="ticket-panel-preview">
        <div className="ticket-preview-embed" style={{ borderLeftColor: color }}>
          ...
          <p>{description || 'Elige una opción para abrir un ticket privado.'}</p>
        ...
        {config.panelMode === 'select' ? <div className="ticket-preview-select"><span>Selecciona el tipo de ticket</span><span>⌄</span></div> : <div className="ticket-preview-buttons">{options.map((option, index) => <button type="button" className="ticket-preview-button" tabIndex="-1" key={option.id || index}>{option.emoji ? <span>{option.emoji}</span> : null}{option.label || `Ticket ${index + 1}`}</button>)}</div>}
...
// Upgrade moderation UI controls.
const moderationPattern = /function ModerationForm\(\{ config, set, resources \}\) \{[\s\S]*?\n\}\n\nfunction AntiRaidForm/;
if (moderationPattern.test(main) && !main.includes('Mensajes para detectar spam')) {
  main = main.replace(moderationPattern, String.raw`function ModerationForm({ config, set, resources }) {
  return <div className="form-card">
    <FormSection title="Anti-spam" description="Detecta ráfagas de mensajes y frena al usuario automáticamente.">
      <ToggleRow title="Anti-spam" description="Elimina el mensaje que dispara el límite y aplica timeout al usuario." checked={!!config.antiSpam} onChange={(value) => set('antiSpam', value)} />
...
const cssMarker = '/* Klvro advanced tickets */';
if (!css.includes(cssMarker)) {
  css += `

${cssMarker}
.ticket-option-stack { display: grid; gap: 12px; }
.ticket-option-editor { padding: 14px; border: 1px solid rgba(255,255,255,.08); border-radius: 12px; background: rgba(255,255,255,.025); }
.ticket-option-editor-head { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 12px; }
.ticket-option-editor-head strong { font-size: 13px; color: #eef0f6; }
.ticket-option-editor-head button { border: 0; background: transparent; color: #f38ba8; font-size: 12px; font-weight: 700; cursor: pointer; }
.ticket-option-editor-head button:disabled { opacity: .35; cursor: not-allowed; }
.ticket-option-fields { display: grid; grid-template-columns: minmax(0, 1fr) 120px; gap: 12px; }
.ticket-add-option { min-height: 40px; padding: 0 14px; border: 1px dashed rgba(88,101,242,.42); border-radius: 10px; background: rgba(88,101,242,.08); color: #cbd0ff; font-weight: 700; cursor: pointer; }
.ticket-add-option:disabled { opacity: .45; cursor: not-allowed; }
.ticket-preview-buttons { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; }
.ticket-preview-buttons .ticket-preview-button { margin-top: 0; }
.ticket-preview-select { display: flex; justify-content: space-between; align-items: center; width: min(100%, 420px); min-height: 40px; margin-top: 8px; padding: 0 12px; border-radius: 4px; background: #1e1f22; color: #b5bac1; font-size: 13px; }
.ticket-preview-reason { margin-top: 10px; color: #949ba4; font-size: 11px; }
.ticket-preview-reason strong { color: #dbdee1; font-weight: 600; }
@media (max-width: 620px) {
  .ticket-option-fields { grid-template-columns: 1fr; }
}
`;
}
...
write(mainPath, main);
write(cssPath, css);
console.log('[Klvro] Tickets avanzados, anti-spam y anti-link aplicados.');
